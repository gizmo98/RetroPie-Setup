...
function install_mupen64plus-testing() {
    for source in *; do
        if [[ -f "$source/projects/unix/Makefile" ]]; then
            ...
            make -C "$source/projects/unix" PREFIX="$md_inst" OPTFLAGS="$CFLAGS" OSD=0 install
        fi
    done
}
...
function configure_mupen64plus-testing() {
    ...
    su "$user" -c "$md_inst/bin/mupen64plus --configdir $rootdir/configs/n64 --datadir $rootdir/configs/n64 --noosd --fullscren --gfx mupen64plus-video-n64.so --audio mupen64plus-audio-omx"
